...
- The Next.js resume pipeline remains local-first. AI provider calls currently go directly from the browser to DeepSeek / OpenAI / Anthropic / Google / OpenRouter.
- `backend/` is a Go HTTP API backed by PostgreSQL. It owns authentication and applies embedded, reversible migrations automatically at startup.
...
For every Go coding, review, debugging, or setup task, load `golang-how-to` first and then every secondary Go skill it routes to for the specific work.
