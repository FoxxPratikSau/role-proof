...
export const extractRevisionReport = (
  rawText: string,
): {
  resume: string;
  report: {
  ...
  try {
    const report = extractJsonFromLLMResponse(reportRaw) as Record<
      string,
      unknown
    >;
    return {
      ...
      report: {
        ...
        unaddressedSuggestions:
          (report.unaddressedSuggestions as string[]) ?? [],
