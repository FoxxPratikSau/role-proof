...
type ChatMessageContent =
  | string
  | Array<ChatTextContentPart | ChatImageContentPart>;
  ...
  deepseek: {
    ...
    models: [
      ...
      },
      {
        ...
        label: "Flash Vision (experimental)",
        desc: "Understands PDF page images for template analysis",
      },
    ],
    ...
    models: [
      {
        value: "gpt-4o",
        label: "GPT-4o",
        desc: "Most capable multimodal model",
      },
      {
        value: "gpt-4o-mini",
        label: "GPT-4o Mini",
...
const buildOpenAICompatBody = (opts: ChatCompletionOptions) => {
  return {
    model: opts.model,
...
const buildDeepSeekBody = (opts: ChatCompletionOptions) => {
  return {
    model: opts.model,
    messages: opts.messages,
    temperature: opts.temperature ?? 0.7,
...
const textContent = (content: ChatMessageContent): string => {
...
const buildAnthropicBody = (opts: ChatCompletionOptions) => {
  ...
  return {
    ...
    system:
      systemMessages.length > 0
        ? systemMessages.map((m) => textContent(m.content)).join("\n\n")
        : undefined,
...
const buildGoogleBody = (opts: ChatCompletionOptions) => {
...
const parseOpenAIUsage = (usage: any) => {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const parseAnthropicUsage = (usage: any) => {
  if (!usage) return undefined;
  return {
...
const parseResponseBody = (
  provider: Provider,
  // Provider payloads intentionally have different shapes and are validated
  // while selecting the fields used by each branch below.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  data: any,
...
const buildUrl = (provider: Provider, opts: ChatCompletionOptions): string => {
...
export const createChatCompletion = async (
  ...
  throw (
    lastError ?? new Error(`${opts.provider} request failed after all retries`)
  );
};
