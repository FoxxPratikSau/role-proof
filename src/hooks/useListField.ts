 ...
 */
export const createNestedStringHelpers = (
  ...
  return {
    updateAt(parentIdx: number, childIdx: number, value: string) {
      ...
      const arr = [
        ...((items[parentIdx][nestedField] as unknown[] | undefined) ?? []),
      ];
      arr[childIdx] = value;
  ...
  return {
    add(parentIdx: number, value: string) {
      ...
      const current: string[] =
        (items[parentIdx].technologies as string[] | undefined) ?? [];
