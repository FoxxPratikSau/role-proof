...
export const checkAlgorithmicConvergence = (
  ...
  // 2. Score delta: less than threshold improvement over the last iteration,
  //    with two consecutive score deltas available so we have a real trend.
  ...
  if (
    current.isConverged &&
    current.score >= MIN_SCORE_FOR_LLM_JUDGMENT &&
    current.atsScore >= ATS_SCORE_CEILING
  ) {
    return {
      isConverged: true,
      reason: "llm_judgment",
